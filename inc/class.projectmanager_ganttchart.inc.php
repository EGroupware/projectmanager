...
class projectmanager_ganttchart extends projectmanager_elements_bo
{
	...
	function &element2bar($pe,$level,$line)
	{
		...
		$indent = $GLOBALS['egw_info']['apps']['projectmanager']['config']['gantt_element_indent'] ? $GLOBALS['egw_info']['apps']['projectmanager']['config']['gantt_element_indent'] : 2;
		$bar = new GanttBar($line,($level ? str_repeat(' ',$level * $indent ) : '').
			$this->text_encode($title).
