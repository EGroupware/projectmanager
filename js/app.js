...
var ProjectmanagerApp = /** @class */ (function (_super) {
	...
	ProjectmanagerApp.prototype.et2_ready = function (et2, name) {
		...
		if (typeof this.views[this.etemplates[et2.name]] != 'undefined') {
			...
			this._bind_sidebox(view_1.sidemenu, function () {
				app.projectmanager.show(view_1.name);
				return false;
			});
		...
		if (current_project) {
			...
			switch (what) {
				...
				case 'gantt':
					...
					if (console.profile)
						console.profile('Gantt');
					if (console.group)
						console.group("Gantt loading PM_ID " + project_id);
					if (console.time)
						console.time("Gantt fetch");
	...
	ProjectmanagerApp.prototype.observer = function (_msg, _app, _id, _type, _msg_type, _links) {
		...
		switch (this.view) {
			...
			case 'gantt':
				var ids = [];
				var gantt_2 = this.views.gantt.etemplate.widgetContainer.getWidgetById('gantt');
