 ...
 * @copyright (c) 2008-9 by Ralf Becker <RalfBecker-AT-outdoor-training.de> and Stefan Becker <StefanBecker-AT-outdoor-training.de>
...
{
	@unlink($csvfile);
	$GLOBALS['egw']->redirect_link('/projectmanager/index.php');
}
...
$template = CreateObject('phpgwapi.Template',common::get_tpl_dir('projectmanager'));
...
$template->set_var("action_url",$GLOBALS['egw']->link("/projectmanager/csv_import.php"));
...
case 'download':
	...
	$template->set_var('lang_project_fieldname',lang('Projectmanager-Fieldname'));
	...
	$template->set_var('lang_field_overwrite',lang('Existing Data will be overwrited if checked'));
	...
	*/
	...
	foreach($csv_fields as $csv_idx => $csv_field)
	{
		...
		if (($def = $defaults[$csv_field]))
		{
			...
			$template->set_var('info_fields',str_replace('="'.$info.'">','="'.$info.'" selected>',$info_name_options));
		}
		else
		{
			$template->set_var('trans','');
			$template->set_var('info_fields',$info_name_options);
		}
	...
	$template->set_var('field_overwrite',get_var('field_overwrite',array('POST'),True)?' checked':'');
...
case 'import':
	...
	for($anz = 0; !$_POST['max'] || $anz < $_POST['max']; ++$anz)
	{
		...
		if (!isset($values['created'])) $values['created'] = time();
		if (!isset($values['creator'])) $values['creator'] = $GLOBALS['egw_info']['user']['account_id'];
		...
		if (isset($values['responsible']))
		{
			...
			{
				if ($user && preg_match('/\[([^\]]+)\]/',$user,$matches)) $user = $matches[1];
		...
		if(!$empty)	// dont import empty contacts
		{
			...
			if (!$_POST['debug'] && ($id = $projectmanager_bo->save($to_write,True,False)))
			{
				$info_link_id = false;
				foreach(array(
